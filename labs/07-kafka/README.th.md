...
**Kafka** คือตัวทดแทนสมัยใหม่โดยตรงของสิ่งที่ **Lab 6 -- Flume**
...
## สำคัญ: Kafka ไม่ได้เก็บข้อมูลใน HDFS

เข้าใจผิดกันง่ายมากในคอร์สที่สร้างรอบ Hadoop เลยขอพูดให้ชัด: **Kafka มี
ระบบเก็บข้อมูลของตัวเอง** มันเขียนไฟล์ log-segment ธรรมดาลงดิสก์ของเครื่อง
โดยตรง ไม่มี NameNode ไม่มี DataNode ไม่มี URI แบบ `hdfs://` อยู่ใน Kafka
เลย พิสูจน์ได้เองหลังทำขั้นตอนที่ 3 (พอ produce ข้อความไปสักสองสามอันแล้ว):
...
คำสั่งแรกจะเห็นไฟล์ `.log`/`.index` จริง ๆ อยู่บน local filesystem ของ
container -- นั่นคือที่ที่ข้อความของคุณเก็บอยู่จริง ส่วนคำสั่งที่สอง (HDFS)
จะไม่เห็นอะไรเกี่ยวกับ Kafka เลย เพราะไม่มีอะไรอยู่ตรงนั้นให้เห็น
(ข้อมูล Kafka ใน sandbox นี้บังเอิญอยู่บน Docker volume เดียวกับที่ HDFS
ใช้เก็บข้อมูลตัวเอง แค่เพื่อไม่ต้องเพิ่ม named volume อีกตัวในการ deploy
เท่านั้น ไม่ได้แปลว่าทั้งสองระบบเกี่ยวข้องกัน -- มันไม่รู้จักกันเลย)

ถ้า**อยาก**ให้ข้อมูล Kafka ไปจบที่ HDFS จริง ๆ นั่นไม่ใช่เรื่องอัตโนมัติ
-- ต้องทำ pipeline แยกต่างหากโดยตั้งใจ (เช่น Kafka Connect's HDFS sink
connector หรือ Spark Structured Streaming job ที่อ่านจาก Kafka แล้วเขียน
ลง HDFS ต่อยอดจากสิ่งที่ Lab 11 ทำกับ socket ดิบ ๆ) หน้าที่ของ Kafka คือ
เป็น log ที่คงทนอยู่ตรงกลาง การเอาข้อมูลเข้า HDFS เป็นขั้นตอนแยกที่ต้อง
ทำเองเสมอ
